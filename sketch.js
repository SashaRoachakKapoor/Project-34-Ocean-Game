...
if(turtlesGroup.isTouching(meatGroup)){
  meatGroup[0].destroy()
}
if(turtlesGroup.isTouching(fishFoodGroup)){
  fishFoodGroup[0].destroy()
}

if(fishesGroup.isTouching(shrimpGroup)){
  shrimpGroup[0].destroy()
} 
if(fishesGroup.isTouching(meatGroup)){
  meatGroup[0].destroy()
}
if(fishesGroup.isTouching(fishFoodGroup)){
  fishFoodGroup[0].destroy()
  score = score+5
}

if(score == "50"){
  text("Well Done! You won the Game!!" , width/2 , 50)
}
...
if(frameCount % 300 === 0){
  fish = createSprite (width , height-250,10,40)
  fish.velocityX = -3
  fish.scale = 0.4;
  fish.lifetime = 500;
  fish.y = Math.round(random(300,height-100));
